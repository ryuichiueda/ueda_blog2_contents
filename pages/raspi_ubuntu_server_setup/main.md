...
　ラズパイにmicroSDをセットしてモニタ、キーボードをつないで立ち上げます。cloud-initというプログラムがいろいろ設定するのにしばらく時間がかかります。下図の表示で止まると思われます。


![](./stop_here.jpg)


　設定が終わっても再起動しないようなので、先に進めなくなった場合「Alt+Ctrl+F6」などで別の仮想コンソール（書籍の2.1節を参考のこと）に切り替えます。「login:」と出ていたら、指定した名前とパスワードでログインします。15分くらいしたら、
```bash
$ sudo poweroff
```
