...
　ラズパイにmicroSDをセットしてモニタ、キーボードをつないで立ち上げます。cloud-initというプログラムがいろいろ設定するのにしばらく時間がかかります。下図の表示で止まると思われます。何分必要かイマイチよくわかっていませんが、15分くらい待ちましょう。


![](./stop_here.jpg)


　待っても終わらず、先に進めなくなった場合「Alt+Ctrl+F6」などで別の仮想コンソール（書籍の2.1節を参考のこと）に切り替えます。「login:」と出ていたら、指定した名前とパスワードでログインします。そして、
```bash
$ sudo poweroff
```
と入力して電源を切るとよいでしょう。ランプがチカチカせず、完全に消えたら電源を外して構いません。これもできなかったら、電源のコネクタを抜いて強制終了し、また挿してみてください。たぶん大丈夫で、「login:」と出てきます。
