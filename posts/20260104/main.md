...
　こんなシェルスクリプト（\verb|./some_task.bash|）を書いてみました。無限ループで、`SECONDS`の値（シェルスクリプトが立ち上がってからの秒数）が変わったら、これまで変数`n`でカウントした数を秒数ともに出力します。
...
　なお、本稿の実験の際はフォークのないシェルスクリプトを書かないといけません。別にプロセスができると、そいつがスケジュリングの対象から漏れてしまいます。あとで使う`chrt`コマンドでエラーが出ます。
