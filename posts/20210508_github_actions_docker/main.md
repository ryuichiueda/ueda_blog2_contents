...
それから、つくばチャレンジのチーム内から「TurtleBot3のシミュレータ側のローンチファイルでGazeboのGUIを切ればよい」という指摘があったのですが、そのほうが素直だと思います。「外部のリポジトリはいじらない」みたいなポリシーでこの作業をはじめてしまったので、Xvfbを使うという選択になりました。
