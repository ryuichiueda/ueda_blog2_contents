...
　今日は一日中コードを書いていた。ただ、下のデモムービーを撮影するときにうまくSLAMができなくて何時間も試行錯誤してしまった。原因は、Ubuntuのネットワーク設定をnetplanで管理するようになってから、WiFiのpower managementを切るのを忘れていた（切らないとブチブチ通信が途切れる）ためで、これはこれで某本の読者に告知しなければならない・・・。
